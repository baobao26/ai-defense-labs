...
- **No test suite for `check-prereqs.sh` or `notify-complete.sh`.** `check-sensitive.sh` and `validate-rule.sh` now both have test suites (`tests/check-sensitive.test.sh`, `tests/validate-rule.test.sh`); the other two have only been exercised manually in-session.
...
- **Bug found and fixed: `validate-rule.sh` always exited 2, even on a valid rule.** Writing `tests/validate-rule.test.sh` against the documented contract (exit 0 = valid, exit 2 = invalid) surfaced that the success path's last line was `echo "...is valid"; exit 2` — a copy-paste leftover from the invalid-case block immediately above it. Every successful validation was being reported to Claude Code as blocking feedback identically to a failure. Fixed to `exit 0`; the new test suite asserts the correct exit code on the valid-rule case so this can't regress silently again.
